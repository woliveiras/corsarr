package application

import (
	"context"

	"github.com/woliveiras/corsarr/internal/hostreadiness"
	runtimeenv "github.com/woliveiras/corsarr/internal/runtime"
)

type EnvironmentStatus struct {
	Platform     string               `json:"platform"`
	Architecture string               `json:"architecture"`
	Host         hostreadiness.Status `json:"host"`
	Runtime      runtimeenv.Status    `json:"runtime"`
}

type EnvironmentService struct {
	runtimeProbe runtimeenv.Probe
	platform     string
	architecture string
	hostChecker  hostreadiness.Checker
}

func NewEnvironmentService(
	runtimeProbe runtimeenv.Probe,
	platform string,
	architecture string,
	hostChecker hostreadiness.Checker,
) *EnvironmentService {
	return &EnvironmentService{
		runtimeProbe: runtimeProbe,
		platform:     platform,
		architecture: architecture,
		hostChecker:  hostChecker,
	}
}

// Status performs read-only environment diagnostics.
func (s *EnvironmentService) Status(ctx context.Context) EnvironmentStatus {
	host := hostreadiness.Status{Ready: true, Issues: []string{}}
	if s.hostChecker != nil {
		host = s.hostChecker.Check(ctx)
	}
	return EnvironmentStatus{
		Platform:     s.platform,
		Architecture: s.architecture,
		Host:         host,
		Runtime:      s.runtimeProbe.Check(ctx),
	}
}
